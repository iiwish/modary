...
// ValidateFlag validates one already-decoded JSON value without materializing
// dependency diagnostics. The supplied budget belongs only to this call.
func (v *Schema) ValidateFlag(root interface{}, limits Budget) (valid bool, err error) {
	budget, err := newEvaluationBudget(limits)
	if err != nil {
		return false, err
	}
	return v.validateManyFlag([]interface{}{root}, budget)
}

// ValidateManyFlag validates roots in order against one shared budget. It
// returns at the first non-match and never materializes dependency diagnostics.
func (v *Schema) ValidateManyFlag(roots []interface{}, limits Budget) (valid bool, err error) {
	budget, err := newEvaluationBudget(limits)
	if err != nil {
		return false, err
	}
	return v.validateManyFlag(roots, budget)
}

func (v *Schema) validateManyFlag(roots []interface{}, budget *evaluationBudget) (valid bool, err error) {
	completed := false
	defer func() {
		if completed {
			return
		...
		_ = recover()
		if limitErr := budget.limitError(); limitErr != nil {
			valid = false
			err = limitErr
			return
		}
	...
	for _, root := range roots {
		result := &Result{flagOnly: true, budget: budget}
...
func (v *subSchema) subValidateWithContext(document interface{}, context *JsonContext, parent *Result) *Result {
	result := parent.child()
...
func (v *subSchema) validateRecursive(currentSubSchema *subSchema, currentNode interface{}, result *Result, context *JsonContext) {
	result.enterFrame()
	defer result.leaveFrame()
	result.consumeWork(1)
	...
	if currentNode == nil {
		...
		if isJSONNumber(currentNode) {
			...
			result.consumeWork(numberWork(value))
			...
			case reflect.Map:
				...
				for property, pSchema := range currentSubSchema.properties {
					result.consumeKey(property)
					nextNode, ok := castCurrentNode[property]
					if ok {
						subContext := result.childContext(property, context)
...
func (v *subSchema) validateSchema(currentSubSchema *subSchema, currentNode interface{}, result *Result, context *JsonContext) {
	...
	if len(currentSubSchema.anyOf) > 0 {
		...
		for _, anyOfSchema := range currentSubSchema.anyOf {
			result.consumeWork(1)
			if !validatedAnyOf {
				validationResult := anyOfSchema.subValidateWithContext(currentNode, context, result)
	...
	if len(currentSubSchema.oneOf) > 0 {
		...
		for _, oneOfSchema := range currentSubSchema.oneOf {
			result.consumeWork(1)
			validationResult := oneOfSchema.subValidateWithContext(currentNode, context, result)
	...
	if len(currentSubSchema.allOf) > 0 {
		...
		for _, allOfSchema := range currentSubSchema.allOf {
			result.consumeWork(1)
			validationResult := allOfSchema.subValidateWithContext(currentNode, context, result)
			if validationResult.Valid() {
				nbValidated++
			}
	...
	if currentSubSchema.not != nil {
		validationResult := currentSubSchema.not.subValidateWithContext(currentNode, context, result)
		if validationResult.Valid() {
	...
	if currentSubSchema.dependencies != nil && len(currentSubSchema.dependencies) > 0 {
		if isKind(currentNode, reflect.Map) {
			object := currentNode.(map[string]interface{})
			for trigger, dependency := range currentSubSchema.dependencies {
				result.consumeKey(trigger)
				...
				case []string:
					for _, dependOnKey := range dependency {
						result.consumeKey(dependOnKey)
						if _, dependencyResolved := object[dependOnKey]; !dependencyResolved {
							result.addInternalError(
	...
	if currentSubSchema._if != nil {
		validationResultIf := currentSubSchema._if.subValidateWithContext(currentNode, context, result)
		if currentSubSchema._then != nil && validationResultIf.Valid() {
			validationResultThen := currentSubSchema._then.subValidateWithContext(currentNode, context, result)
		...
		if currentSubSchema._else != nil && !validationResultIf.Valid() {
			validationResultElse := currentSubSchema._else.subValidateWithContext(currentNode, context, result)
...
func (v *subSchema) validateCommon(currentSubSchema *subSchema, value interface{}, result *Result, context *JsonContext) {
	...
	if currentSubSchema._const != nil {
		vString, err := marshalWithoutNumberBudget(value, result.budget)
		if err != nil {
			result.addInternalError(new(InternalError), context, value, ErrorDetails{"error": err})
			return
		}
		result.consumeBytes(uint64(len(*vString) + len(*currentSubSchema._const)))
	...
	if len(currentSubSchema.enum) > 0 {
		...
		result.consumeWork(uint64(len(currentSubSchema.enum)))
		for _, candidate := range currentSubSchema.enum {
			result.consumeBytes(uint64(len(*vString) + len(candidate)))
		}
...
func (v *subSchema) validateArray(currentSubSchema *subSchema, value []interface{}, result *Result, context *JsonContext) {
	...
	// A single items schema applies independently to every array element.
	if currentSubSchema.itemsChildrenIsSingleSchema {
		for i := range value {
			result.consumeWork(1)
			subContext := result.indexContext(i, context)
			validationResult := currentSubSchema.itemsChildren[0].subValidateWithContext(value[i], subContext, result)
			result.mergeErrors(validationResult)
		}
	} else {
		if currentSubSchema.itemsChildren != nil && len(currentSubSchema.itemsChildren) > 0 {
			...
			for i := 0; i != nbItems && i != nbValues; i++ {
				result.consumeWork(1)
				subContext := result.indexContext(i, context)
				validationResult := currentSubSchema.itemsChildren[i].subValidateWithContext(value[i], subContext, result)
				result.mergeErrors(validationResult)
			}
			...
			if nbItems < nbValues {
				...
				case *subSchema:
					...
					for i := nbItems; i != nbValues; i++ {
						result.consumeWork(1)
						subContext := result.indexContext(i, context)
						validationResult := additionalItemSchema.subValidateWithContext(value[i], subContext, result)
						result.mergeErrors(validationResult)
					}
				}
			}
	...
	if currentSubSchema.uniqueItems {
		...
		for j, v := range value {
			result.consumeWork(1)
			vString, err := marshalWithoutNumberBudget(v, result.budget)
			if err != nil {
	...
	if currentSubSchema.contains != nil {
		...
		for i, v := range value {
			...
			validationResult := currentSubSchema.contains.subValidateWithContext(v, subContext, result)
			if validationResult.Valid() {
				validatedOne = true
				break
			} else {
...
func (v *subSchema) validateObject(currentSubSchema *subSchema, value map[string]interface{}, result *Result, context *JsonContext) {
	...
	for _, requiredProperty := range currentSubSchema.required {
		result.consumeKey(requiredProperty)
	...
	for pk, propertyValue := range value {
		result.consumeWork(1)

		// Check whether this property is described by "properties"
		result.consumeKey(pk)
		_, found := currentSubSchema.properties[pk]

		//  Check whether this property is described by "patternProperties"
		ppMatch := v.validatePatternProperty(currentSubSchema, pk, propertyValue, result, context)
		...
		if !found && !ppMatch {
			...
			case *subSchema:
				validationResult := ap.subValidateWithContext(propertyValue, result.childContext(pk, context), result)
				result.mergeErrors(validationResult)
			}
		}
	}
	...
	if currentSubSchema.propertyNames != nil {
		for pk := range value {
			result.consumeWork(1)
			validationResult := currentSubSchema.propertyNames.subValidateWithContext(pk, context, result)
			if !validationResult.Valid() {
...
func (v *subSchema) validatePatternProperty(currentSubSchema *subSchema, key string, value interface{}, result *Result, context *JsonContext) bool {
	...
	for pk, pv := range currentSubSchema.patternProperties {
		result.consumeWork(regexpWork(uint64(len(pk)), uint64(len(key))))
		if currentSubSchema.patternExpressions[pk].MatchString(key) {
			validated = true
			subContext := result.childContext(key, context)
			validationResult := pv.subValidateWithContext(value, subContext, result)
			result.mergeErrors(validationResult)
		}
	}
...
func (v *subSchema) validateString(currentSubSchema *subSchema, value interface{}, result *Result, context *JsonContext) {
	...
	// minLength & maxLength:
	stringLength := -1
	if currentSubSchema.minLength != nil || currentSubSchema.maxLength != nil {
		result.consumeBytes(uint64(len(stringValue)))
		stringLength = utf8.RuneCount([]byte(stringValue))
	}
	if currentSubSchema.minLength != nil {
		if stringLength < int(*currentSubSchema.minLength) {
			result.addInternalError(
	...
	if currentSubSchema.maxLength != nil {
		if stringLength > int(*currentSubSchema.maxLength) {
			result.addInternalError(
	...
	if currentSubSchema.pattern != nil {
		result.consumeWork(regexpWork(
			uint64(len(currentSubSchema.pattern.String())),
			uint64(len(stringValue)),
		))
...
func (v *subSchema) validateNumber(currentSubSchema *subSchema, value interface{}, result *Result, context *JsonContext) {
	...
	if currentSubSchema.multipleOf != nil {
		result.consumeWork(currentSubSchema.multipleOfWork)
	...
	if currentSubSchema.maximum != nil {
		result.consumeWork(currentSubSchema.maximumWork)
	...
	if currentSubSchema.exclusiveMaximum != nil {
		result.consumeWork(currentSubSchema.exclusiveMaxWork)
	...
	if currentSubSchema.minimum != nil {
		result.consumeWork(currentSubSchema.minimumWork)
	...
	if currentSubSchema.exclusiveMinimum != nil {
		result.consumeWork(currentSubSchema.exclusiveMinWork)
