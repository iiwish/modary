...
const draft7MetaSchemaSHA256 = "cdb776f18000574676e47af000f2d64650863dfb71fa0097c50286fee8a8d0b0"

type draft7MetaSchemaCache struct {
	once     sync.Once
	compiled *Schema
	err      error
}

var pinnedDraft7MetaSchema draft7MetaSchemaCache
...
// NewLocalSchema compiles an already-decoded document with an explicit draft.
// Resolution is restricted to JSON Pointers within that root document.
func NewLocalSchema(document interface{}, draft Draft) (*Schema, error) {
	loader := NewSchemaLoader()
	loader.AutoDetect = false
	loader.Draft = draft
	return loader.Compile(NewRawLoader(document))
}

// NewDraft7MetaSchema compiles the engine's pinned Draft 7 metaschema without
// consulting a file, network, or caller-provided loader.
func NewDraft7MetaSchema() (*Schema, error) {
	return pinnedDraft7MetaSchema.load()
}

func (cache *draft7MetaSchemaCache) load() (*Schema, error) {
	cache.once.Do(func() {
		source := drafts.GetMetaSchema(drafts.GetSchemaURL(Draft7))
		if source == "" {
			cache.err = errors.New("Draft 7 metaschema is not embedded")
			return
		}
		digest := fmt.Sprintf("%x", sha256.Sum256([]byte(source)))
		if digest != draft7MetaSchemaSHA256 {
			cache.err = fmt.Errorf(
				"Draft 7 metaschema digest %s does not match pinned digest %s",
				digest,
				draft7MetaSchemaSHA256,
			)
			return
		}
		document, err := decodeJSONUsingNumber(strings.NewReader(source))
		if err != nil {
			cache.err = fmt.Errorf("decode pinned Draft 7 metaschema: %w", err)
			return
		}
		cache.compiled, cache.err = NewLocalSchema(document, Draft7)
	})
	return cache.compiled, cache.err
}
...
func (d *Schema) parseSchema(documentNode interface{}, currentSchema *subSchema) error {
	...
	if existsMapKey(m, KEY_PATTERN_PROPERTIES) {
		if isKind(m[KEY_PATTERN_PROPERTIES], reflect.Map) {
			...
			if len(patternPropertiesMap) > 0 {
				...
				currentSchema.patternExpressions = make(map[string]*regexp.Regexp)
				for k, v := range patternPropertiesMap {
					...
					currentSchema.patternExpressions[k] = pattern
				}
			}
		} else {
			return errors.New(formatErrorDescription(
	...
	if existsMapKey(m, KEY_MULTIPLE_OF) {
		...
		currentSchema.multipleOfWork = rationalWork(multipleOfValue)
	}
	...
	if existsMapKey(m, KEY_MINIMUM) {
		...
		currentSchema.minimumWork = rationalWork(minimumValue)
	}
	...
	if existsMapKey(m, KEY_EXCLUSIVE_MINIMUM) {
		...
		case Draft4:
			...
			if m[KEY_EXCLUSIVE_MINIMUM].(bool) {
				...
				currentSchema.exclusiveMinWork = currentSchema.minimumWork
				currentSchema.minimum = nil
				currentSchema.minimumWork = 0
			}
		case Hybrid:
			if isKind(m[KEY_EXCLUSIVE_MINIMUM], reflect.Bool) {
				if currentSchema.minimum == nil {
				...
				currentSchema.exclusiveMinWork = rationalWork(currentSchema.exclusiveMinimum)
			} else {
				return errors.New(formatErrorDescription(
					Locale.InvalidType(),
					ErrorDetails{
	...
	if existsMapKey(m, KEY_MAXIMUM) {
		...
		currentSchema.maximumWork = rationalWork(maximumValue)
	}
	...
	if existsMapKey(m, KEY_EXCLUSIVE_MAXIMUM) {
		...
		case Draft4:
			...
			if m[KEY_EXCLUSIVE_MAXIMUM].(bool) {
				...
				currentSchema.exclusiveMaxWork = currentSchema.maximumWork
				currentSchema.maximum = nil
				currentSchema.maximumWork = 0
			}
		case Hybrid:
			if isKind(m[KEY_EXCLUSIVE_MAXIMUM], reflect.Bool) {
				if currentSchema.maximum == nil {
				...
				currentSchema.exclusiveMaxWork = rationalWork(currentSchema.exclusiveMaximum)
			} else {
				return errors.New(formatErrorDescription(
					Locale.InvalidType(),
					ErrorDetails{
...
func (d *Schema) parseProperties(documentNode interface{}, currentSchema *subSchema) error {
	...
	m := documentNode.(map[string]interface{})
	currentSchema.properties = make(map[string]*subSchema, len(m))
	for k := range m {
		schemaProperty := k
		newSchema := &subSchema{property: schemaProperty, parent: currentSchema, ref: currentSchema.ref}
		currentSchema.properties[schemaProperty] = newSchema
