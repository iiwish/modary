...
func (v *Result) child() *Result {
	return &Result{flagOnly: v.flagOnly, budget: v.budget}
}

func (v *Result) consumeWork(units uint64) {
	if v != nil {
		v.budget.consumeWork(units)
	}
}

func (v *Result) consumeBytes(bytes uint64) {
	// One work unit represents one byte scanned or one scalar operation.
	v.consumeWork(bytes)
}

func (v *Result) consumeKey(key string) {
	v.consumeWork(saturatingAdd(1, uint64(len(key))))
}

func (v *Result) enterFrame() {
	if v != nil {
		v.budget.enterFrame()
	}
}

func (v *Result) leaveFrame() {
	if v != nil {
		v.budget.leaveFrame()
	}
}

func (v *Result) childContext(field string, parent *JsonContext) *JsonContext {
	if v != nil && v.flagOnly {
		return nil
	}
	return NewJsonContext(field, parent)
}

func (v *Result) indexContext(index int, parent *JsonContext) *JsonContext {
	if v != nil && v.flagOnly {
		return nil
	}
	return NewJsonContext(strconv.Itoa(index), parent)
}
