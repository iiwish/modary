...
func TestSQLiteStoresEnforcePublishedActionJSONBoundaryMatrix(t *testing.T) {
	services := startTestServices(t, Options{Path: filepath.Join(t.TempDir(), "modary.db")})
	ctx := context.Background()
	for index, boundary := range sqliteJSONBoundaries() {
...
type sqliteJSONBoundary struct {
	name   string
	value  json.RawMessage
	within bool
}

func sqliteJSONBoundaries() []sqliteJSONBoundary {
	return []sqliteJSONBoundary{
		...
		{name: "depth exact", value: sqliteNestedJSON(action.MaxJSONNestingDepth), within: true},
		{name: "depth above", value: sqliteNestedJSON(action.MaxJSONNestingDepth + 1)},
		{name: "nodes exact", value: sqliteArrayJSON(action.MaxJSONValueNodes - 1), within: true},
		{name: "nodes above", value: sqliteArrayJSON(action.MaxJSONValueNodes)},
		{name: "number exact", value: sqliteNumberJSON(action.MaxJSONNumberBytes), within: true},
		{name: "number above", value: sqliteNumberJSON(action.MaxJSONNumberBytes + 1)},
	}
}

func sqliteNestedJSON(depth int) json.RawMessage {
	return json.RawMessage(strings.Repeat("[", depth) + "0" + strings.Repeat("]", depth))
}

func sqliteArrayJSON(values int) json.RawMessage {
	return json.RawMessage("[" + strings.TrimSuffix(strings.Repeat("0,", values), ",") + "]")
}

func sqliteNumberJSON(bytes int) json.RawMessage {
	return json.RawMessage("1" + strings.Repeat("0", bytes-1))
}

func TestSQLiteStoresAcceptCanonicalMaximumPolicyFingerprint(t *testing.T) {
	services := startTestServices(t, Options{Path: filepath.Join(t.TempDir(), "modary.db")})
	ctx := context.Background()
...
func TestSQLitePlanFingerprintFailsClosedAtAPIAndSchemaBoundaries(t *testing.T) {
	services := startTestServices(t, Options{Path: filepath.Join(t.TempDir(), "modary.db")})
	ctx := context.Background()
	plan := validPlan()
	...
	if _, err := services.db.Exec(
		`UPDATE modary_action_plan SET decision_fingerprint = '' WHERE plan_hash = ?`,
		plan.Hash,
	); err == nil {
		t.Fatal("SQLite schema accepted an empty plan decision fingerprint")
	}
...
func TestPlanAndIdempotencyRecordsSurviveRestartExactly(t *testing.T) {
	path := filepath.Join(t.TempDir(), "durable", "modary.db")
...
func TestTransactionManagerCommitsRollsBackAndNests(t *testing.T) {
	services := startTestServices(t, Options{Path: filepath.Join(t.TempDir(), "modary.db")})
	ctx := context.Background()
	if _, err := services.db.Exec(`CREATE TABLE transaction_probe (value TEXT PRIMARY KEY) STRICT`); err != nil {
		t.Fatal(err)
	}
...
func TestRootTransactionNilPanicRollsBackAndPropagates(t *testing.T) {
	services := startTestServices(t, Options{Path: filepath.Join(t.TempDir(), "modary.db")})
	ctx := context.Background()
	if _, err := services.db.Exec(`CREATE TABLE transaction_nil_panic_probe (value TEXT PRIMARY KEY) STRICT`); err != nil {
		t.Fatal(err)
	}
...
func TestCorruptedRowsFailClosed(t *testing.T) {
	...
	if _, err := services.db.Exec(`UPDATE modary_action_plan SET impact_resources_json = '[1]' WHERE plan_hash = ?`, plan.Hash); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := services.db.Exec(`
		...
		SET result_references_json = '[{"kind":"ignored","kind":"write","id":"one"}]'
	...
	if _, err := services.idempotency.Lookup(context.Background(), record); err == nil || !strings.Contains(err.Error(), "duplicate JSON object property") {
		t.Fatalf("corrupted reference error = %v", err)
	}
}

func TestStoredActionJSONIsBoundedBeforeSQLiteScan(t *testing.T) {
	services := startTestServices(t, Options{Path: filepath.Join(t.TempDir(), "modary.db")})
	ctx := context.Background()
	plan := validPlan()
	if err := services.plans.Save(ctx, plan); err != nil {
	...
	if _, err := services.db.ExecContext(ctx, `UPDATE modary_action_plan SET payload_json = ? WHERE plan_hash = ?`, oversized, plan.Hash); err == nil {
		t.Fatal("SQLite schema accepted an oversized plan payload")
	}
	if _, err := services.db.ExecContext(ctx, `UPDATE modary_action_plan SET impact_resources_json = ? WHERE plan_hash = ?`, string(oversizedCollection), plan.Hash); err == nil {
		t.Fatal("SQLite schema accepted oversized plan impact resources")
	}
	if _, err := services.db.ExecContext(ctx, `UPDATE modary_action_plan SET payload_json = ? WHERE plan_hash = ?`, oversizedMultibyte, plan.Hash); err == nil {
		t.Fatal("SQLite schema counted a multibyte plan payload by characters instead of bytes")
	}
	if _, err := services.db.ExecContext(ctx, `
		UPDATE modary_action_idempotency SET impact_resources_json = ?
		WHERE scope_kind = ? AND scope_id = ? AND actor_id = ? AND actor_type = ?
		  AND action_id = ? AND idempotency_key = ?`,
		...
		t.Fatal("SQLite schema accepted oversized idempotency impact resources")
	}
	if _, err := services.db.ExecContext(ctx, `
		...
		WHERE scope_kind = ? AND scope_id = ? AND actor_id = ? AND actor_type = ?
		  AND action_id = ? AND idempotency_key = ?`,
		string(oversizedCollection), record.Scope.Kind, record.Scope.ID, record.ActorID, record.ActorType, record.ActionID, record.Key); err == nil {
		t.Fatal("SQLite schema accepted oversized idempotency references")
	}
	if _, err := services.db.ExecContext(ctx, `
		UPDATE modary_action_idempotency SET result_data_json = ?
		WHERE scope_kind = ? AND scope_id = ? AND actor_id = ? AND actor_type = ?
		  AND action_id = ? AND idempotency_key = ?`,
		oversizedMultibyte, record.Scope.Kind, record.Scope.ID, record.ActorID, record.ActorType, record.ActionID, record.Key); err == nil {
		t.Fatal("SQLite schema counted a multibyte idempotency result by characters instead of bytes")
	}
	...
	if _, err := connection.ExecContext(ctx, `UPDATE modary_action_plan SET impact_resources_json = ? WHERE plan_hash = ?`, string(oversizedCollection), plan.Hash); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := connection.ExecContext(ctx, `UPDATE modary_action_plan SET impact_resources_json = ? WHERE plan_hash = ?`, string(planResources), plan.Hash); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := connection.ExecContext(ctx, `UPDATE modary_action_plan SET payload_json = ? WHERE plan_hash = ?`, oversized, plan.Hash); err != nil {
		t.Fatal(err)
	}
	if _, err := connection.ExecContext(ctx, `
		UPDATE modary_action_idempotency SET result_data_json = ?
	...
	if _, err := connection.ExecContext(ctx, `UPDATE modary_action_plan SET payload_json = ?, actor_id = ? WHERE plan_hash = ?`,
	...
	if _, err := connection.ExecContext(ctx, `
		UPDATE modary_action_idempotency
		SET result_data_json = ?, result_summary = ?
		WHERE scope_kind = ? AND scope_id = ? AND actor_id = ? AND actor_type = ?
		  AND action_id = ? AND idempotency_key = ?`,
