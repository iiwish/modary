...
func TestModuleIsExplicitAndSideEffectFree(t *testing.T) {
	...
	if manifest.ID != ModuleID || manifest.Type != module.ModuleTypeAdapter || !reflect.DeepEqual(manifest.Provides, []module.Capability{
		module.CapabilityDatabase, module.CapabilityTasks, module.CapabilityTaskInspection,
	}) {
...
func TestModuleRejectsInvalidOptionsBeforeSideEffects(t *testing.T) {
	tests := []Options{
		...
		{URL: defaultTestURL, QueueSchema: strings.Repeat("q", 47)},
...
func TestModuleAcceptsMaximumRiverQueueSchemaLength(t *testing.T) {
	if _, err := Module(Options{
		URL:               defaultTestURL,
		ApplicationSchema: "application_schema",
		QueueSchema:       strings.Repeat("q", maxRiverQueueSchemaBytes),
	}); err != nil {
		t.Fatalf("Module() rejected maximum River queue schema length: %v", err)
	}
}
...
func TestTaskInspectorIsBoundedFilterableAndCursorBased(t *testing.T) {
	services := startTestServices(t)
	inspector, ok := services.tasks.(task.Inspector)
	if !ok {
		t.Fatal("task service does not expose the declared Inspector contract")
	}
	err := services.control.WithinTransaction(context.Background(), func(txCtx context.Context) error {
		for _, request := range []task.Request{
			{Kind: "probe.first", Queue: "priority"},
			{Kind: "probe.second", Queue: "default"},
			{Kind: "probe.third", Queue: "priority"},
		} {
			if _, err := services.tasks.Enqueue(txCtx, request); err != nil {
				return err
			}
		}
		return nil
	...
	first, err := inspector.List(context.Background(), task.ListOptions{Limit: 2})
	if err != nil {
		t.Fatal(err)
	}
	if len(first.Tasks) != 2 || first.NextBeforeID == 0 || first.Tasks[0].ID <= first.Tasks[1].ID {
		t.Fatalf("first page = %#v", first)
	}
	for _, item := range first.Tasks {
		if item.State != task.StateQueued {
			t.Fatalf("provider state leaked through public task summary: %#v", item)
		}
	}
	second, err := inspector.List(context.Background(), task.ListOptions{Limit: 2, BeforeID: first.NextBeforeID})
	if err != nil || len(second.Tasks) != 1 || second.Tasks[0].ID >= first.NextBeforeID {
		t.Fatalf("second page = %#v, error = %v", second, err)
	}
	filtered, err := inspector.List(context.Background(), task.ListOptions{Queue: "priority"})
	if err != nil || len(filtered.Tasks) != 2 {
		t.Fatalf("filtered page = %#v, error = %v", filtered, err)
	}
	for _, item := range filtered.Tasks {
		if item.Queue != "priority" {
			t.Fatalf("queue filter leaked %#v", item)
		}
	}
	queued, err := inspector.List(context.Background(), task.ListOptions{State: task.StateQueued})
	if err != nil || len(queued.Tasks) != 3 {
		t.Fatalf("canonical state filter = %#v, error = %v", queued, err)
	}
}

func TestTaskInspectorInstallsIndexesForEveryFilterShape(t *testing.T) {
	services := startTestServices(t)
	rows, err := services.db.QueryContext(context.Background(), `
		SELECT indexname
		FROM pg_indexes
		WHERE schemaname = $1 AND tablename = 'river_job'`, services.options.QueueSchema)
	if err != nil {
		t.Fatal(err)
	}
	defer rows.Close()
	...
	for _, name := range []string{
		"modary_task_inspection_queue_id_idx",
		"modary_task_inspection_state_id_idx",
		"modary_task_inspection_state_queue_id_idx",
	} {
		if _, ok := indexes[name]; !ok {
			t.Errorf("task inspection index %q was not installed; indexes = %v", name, indexes)
		}
	}
}

func TestTaskInspectorFilterQueriesUseDedicatedIndexes(t *testing.T) {
	services := startTestServices(t)
	tx, err := services.db.BeginTx(context.Background(), nil)
	if err != nil {
		t.Fatal(err)
	}
	defer tx.Rollback()
	if _, err := tx.Exec(`SET LOCAL enable_seqscan = off; SET LOCAL enable_bitmapscan = off`); err != nil {
		t.Fatal(err)
	}
	tests := []struct {
		name       string
		...
		{name: "queue", options: task.ListOptions{Queue: "priority", Limit: 50}, index: "modary_task_inspection_queue_id_idx"},
		{name: "state", options: task.ListOptions{State: task.StateQueued, Limit: 50}, riverState: "available", index: "modary_task_inspection_state_id_idx"},
		{name: "state and queue", options: task.ListOptions{Queue: "priority", State: task.StateQueued, Limit: 50}, riverState: "available", index: "modary_task_inspection_state_queue_id_idx"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			query, arguments := taskInspectionQuery(services.options.QueueSchema, test.options, test.riverState)
			rows, err := tx.Query(`EXPLAIN (COSTS OFF) `+query, arguments...)
			if err != nil {
				t.Fatal(err)
			}
			var lines []string
			...
			if !strings.Contains(plan, test.index) || strings.Contains(plan, "Seq Scan") || strings.Contains(plan, "Sort") {
				t.Fatalf("task inspection plan does not use %s without a scan or sort:\n%s", test.index, plan)
			}
		})
	}
}
