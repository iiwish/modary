...
func TestPostgresNestedTransactionSuccessJoinsOuterUnit(t *testing.T) {
...
func TestPostgresNestedTransactionFailureMarksOuterRollbackOnly(t *testing.T) {
...
func TestPostgresNestedTransactionPanicMarksOuterRollbackOnly(t *testing.T) {
	for _, test := range []struct {
		name  string
		panic func()
	}{
...
func insertNestedProbe(services testServices, ctx context.Context, value string) error {
	...
	_, err = executor.ExecContext(ctx, `INSERT INTO nested_transaction_probe(value) VALUES ($1)`, value)
	return err
}
