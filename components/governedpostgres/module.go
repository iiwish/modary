// Package governedpostgres provides Modary's governed PostgreSQL profile and
...
const (
	schemaProfileTable    = databasecontrol.SchemaProfileTable
	schemaRoleApplication = databasecontrol.SchemaRoleApplication
	schemaRoleQueue       = databasecontrol.SchemaRoleQueue
)
...
func start(ctx context.Context, scope module.Scope, options Options, config *pgx.ConnConfig) error {
	...
	if err := module.Provide[task.Service](scope, module.Tasks(), tasks); err != nil {
		return err
	}
	return module.Provide[task.Inspector](scope, module.TaskInspector(), tasks)
}
...
func prepareSchemasAndQueue(
	...
	if err := createTaskInspectionIndexes(ctx, lockConnection, options.QueueSchema); err != nil {
		return err
	}
...
func createTaskInspectionIndexes(ctx context.Context, connection *pgx.Conn, schema string) error {
	table := quoteIdentifier(schema) + `.river_job`
	indexes := []struct {
		name    string
		columns string
	}{
		{name: "modary_task_inspection_queue_id_idx", columns: "queue, id DESC"},
		{name: "modary_task_inspection_state_id_idx", columns: "state, id DESC"},
		{name: "modary_task_inspection_state_queue_id_idx", columns: "state, queue, id DESC"},
	}
	for _, index := range indexes {
		statement := `CREATE INDEX IF NOT EXISTS ` + quoteIdentifier(index.name) + ` ON ` + table + ` (` + index.columns + `)`
		if _, err := connection.Exec(ctx, statement); err != nil {
			return fmt.Errorf("create PostgreSQL task inspection index %q: %w", index.name, err)
		}
	}
	return nil
}
...
func bindSchemaPair(ctx context.Context, connection *pgx.Conn, application, queue string) error {
	...
	for _, binding := range bindings {
		...
		if _, err := tx.Exec(ctx, `CREATE TABLE IF NOT EXISTS `+table+` (
			...
			peer_schema TEXT CHECK (peer_schema IS NULL OR octet_length(peer_schema) BETWEEN 1 AND 63)
		)`); err != nil {
		...
		err := tx.QueryRow(ctx, `SELECT
			...
			CASE WHEN peer_schema IS NULL OR octet_length(peer_schema) <= 63 THEN peer_schema END
			FROM `+table+` WHERE profile_id = 1`).Scan(&boundRole, &boundPeer)
		if errors.Is(err, pgx.ErrNoRows) {
			if _, err := tx.Exec(ctx, `INSERT INTO `+table+` (profile_id, schema_role, peer_schema)
				VALUES (1, $1, $2)`, binding.role, binding.peer); err != nil {
				return fmt.Errorf("bind PostgreSQL schema profile for %q: %w", binding.schema, err)
			}
			boundRole = sql.NullString{String: binding.role, Valid: true}
			boundPeer = sql.NullString{String: binding.peer, Valid: true}
		} else if err != nil {
			return fmt.Errorf("verify PostgreSQL schema profile binding for %q: %w", binding.schema, err)
		}
		if binding.role == schemaRoleApplication && boundRole.Valid && boundRole.String == schemaRoleApplication && !boundPeer.Valid {
			result, err := tx.Exec(ctx, `UPDATE `+table+` SET peer_schema = $1
				WHERE profile_id = 1 AND schema_role = $2 AND peer_schema IS NULL`, binding.peer, schemaRoleApplication)
			if err != nil {
				return fmt.Errorf("upgrade PostgreSQL application schema binding for %q: %w", binding.schema, err)
			}
