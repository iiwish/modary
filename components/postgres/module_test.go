...
import (
	...
	"github.com/iiwish/modary/components/postgres/internal/testpostgres"
...
func TestModuleCreatesOnlyApplicationSchema(t *testing.T) {
	...
	profileQuery := `SELECT schema_role, peer_schema FROM ` + quoteIdentifier(config.ApplicationSchema) +
		`.` + quoteIdentifier(databasecontrol.SchemaProfileTable) + ` WHERE profile_id = 1`
	if err := connection.QueryRow(context.Background(), profileQuery).Scan(&role, &peer); err != nil {
		t.Fatal(err)
	}
	if role != databasecontrol.SchemaRoleApplication || peer.Valid {
		t.Fatalf("ordinary schema binding = %q, %#v; want unpaired application", role, peer)
	}
