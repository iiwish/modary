...
func prepareSchema(ctx context.Context, config *pgx.ConnConfig, schema string, lockKey int64) (err error) {
	...
	if err := bindApplicationSchema(ctx, connection, schema); err != nil {
		return err
	}
	var unlocked bool
...
func bindApplicationSchema(ctx context.Context, connection *pgx.Conn, schema string) error {
	tx, err := connection.Begin(ctx)
	if err != nil {
		return fmt.Errorf("begin PostgreSQL schema role binding: %w", err)
	}
	defer tx.Rollback(ctx)
	table := quoteIdentifier(schema) + `.` + quoteIdentifier(databasecontrol.SchemaProfileTable)
	if _, err := tx.Exec(ctx, `CREATE TABLE IF NOT EXISTS `+table+` (
		...
		return fmt.Errorf("create PostgreSQL schema role binding for %q: %w", schema, err)
	}
	var role, peer sql.NullString
	var peerMissing bool
	err = tx.QueryRow(ctx, `SELECT
		...
		peer_schema IS NULL
		FROM `+table+` WHERE profile_id = 1`).Scan(&role, &peer, &peerMissing)
	if errors.Is(err, pgx.ErrNoRows) {
		if _, err := tx.Exec(ctx, `INSERT INTO `+table+` (profile_id, schema_role, peer_schema)
			VALUES (1, $1, NULL)`, databasecontrol.SchemaRoleApplication); err != nil {
			return fmt.Errorf("bind PostgreSQL application schema %q: %w", schema, err)
		}
		role = sql.NullString{String: databasecontrol.SchemaRoleApplication, Valid: true}
		peerMissing = true
	} else if err != nil {
		return fmt.Errorf("inspect PostgreSQL schema role binding for %q: %w", schema, err)
	}
	if !role.Valid || role.String != databasecontrol.SchemaRoleApplication ||
		(peerMissing && peer.Valid) || (!peerMissing && (!peer.Valid || peer.String == "")) {
		return fmt.Errorf("PostgreSQL schema %q is already bound to a different role", schema)
	}
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("commit PostgreSQL schema role binding: %w", err)
	}
	return nil
}
