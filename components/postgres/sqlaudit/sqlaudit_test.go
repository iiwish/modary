...
func TestReaderIsScopeBoundedAndCursorBased(t *testing.T) {
	_, store := openHook(t)
	now := time.Now().UTC()
	for index, executionScope := range []scope.Execution{
		scope.Must("account", "account-1"),
		scope.Must("account", "account-2"),
		scope.Must("account", "account-1"),
	} {
		event := completeEvent(now.Add(time.Duration(index) * time.Second))
		event.RequestID = fmt.Sprintf("request-%d", index+1)
		event.Scope = executionScope
		if err := store.Record(context.Background(), event); err != nil {
			t.Fatal(err)
		}
	}
	first, err := store.List(context.Background(), audit.ListOptions{Scope: scope.Must("account", "account-1"), Limit: 1})
	if err != nil {
		t.Fatal(err)
	}
	if len(first.Events) != 1 || first.NextBeforeID == 0 || first.Events[0].Scope.ID != "account-1" {
		t.Fatalf("first page = %#v", first)
	}
	second, err := store.List(context.Background(), audit.ListOptions{
		Scope: scope.Must("account", "account-1"), Limit: 1, BeforeID: first.NextBeforeID,
	})
	if err != nil || len(second.Events) != 1 || second.Events[0].ID >= first.NextBeforeID || second.NextBeforeID != 0 {
		t.Fatalf("second page = %#v, error = %v", second, err)
	}
	if _, err := store.List(context.Background(), audit.ListOptions{}); err == nil {
		t.Fatal("reader accepted an unbound scope")
	}
}
...
func TestAuditInspectionQueryUsesCursorSpecificShapes(t *testing.T) {
	executionScope := scope.Must("account", "account-1")
	withoutCursor, withoutArguments := auditInspectionQuery(audit.ListOptions{
		Scope: executionScope, Limit: 50,
	})
	if strings.Contains(withoutCursor, " OR ") || strings.Contains(withoutCursor, "event_id <") || len(withoutArguments) != 3 {
		t.Fatalf("no-cursor audit query is not a dedicated shape: %q %#v", withoutCursor, withoutArguments)
	}
	withCursor, withArguments := auditInspectionQuery(audit.ListOptions{
		Scope: executionScope, Limit: 50, BeforeID: 9007199254740993,
	})
	if strings.Contains(withCursor, " OR ") || !strings.Contains(withCursor, "event_id < $3") || len(withArguments) != 4 {
		t.Fatalf("cursor audit query is not a dedicated shape: %q %#v", withCursor, withArguments)
	}
}

func TestAuditInspectionQueriesUseScopeCursorIndex(t *testing.T) {
	db, _ := openHook(t)
	...
	for _, options := range []audit.ListOptions{
		{Scope: scope.Must("account", "account-1"), Limit: 50},
		{Scope: scope.Must("account", "account-1"), Limit: 50, BeforeID: 9007199254740993},
	} {
		query, arguments := auditInspectionQuery(options)
		...
		if !strings.Contains(plan, "modary_audit_event_scope") || strings.Contains(plan, "Seq Scan") || strings.Contains(plan, "Sort") {
			t.Fatalf("audit inspection plan does not use the scope cursor index without a scan or sort:\n%s", plan)
		}
	}
}
