...
const (
	...
	identityStoreSynchronizationTimeout = 10 * time.Second
)
...
func TestManifestPublishesOnlyExplicitCredentialCapabilities(t *testing.T) {
	tests := []struct {
		name     string
		options  Options
		provides []module.Capability
	}{
		{name: "principal only", options: Options{}, provides: []module.Capability{
			module.CapabilityIdentity, module.CapabilitySessions,
		}},
		{name: "password", options: Options{
			PasswordCredentials: []PasswordCredential{{ActorID: "person-one", Username: "person@example.test", Password: testPassword}},
		}, provides: []module.Capability{
			module.CapabilityIdentity, module.CapabilityPasswords, module.CapabilitySessions,
		}},
		{name: "bearer", options: Options{
			BearerTokens: []BearerToken{{TokenID: "service-one", ActorID: "person-one", Token: testToken}},
		}, provides: []module.Capability{
			module.CapabilityIdentity, module.CapabilityBearers, module.CapabilitySessions,
		}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			registration, err := Module(test.options)
			if err != nil {
				t.Fatal(err)
			}
			if !slices.Equal(registration.Definition.Manifest.Provides, test.provides) {
				t.Fatalf("provides = %v, want %v", registration.Definition.Manifest.Provides, test.provides)
			}
		})
	}
}
...
func TestCredentiallessPrincipalCanBeResolvedWithoutLoginSurface(t *testing.T) {
	...
	if actor.ID != "worker-one" || actor.Type != "service" {
...
func TestExplicitCredentialsAuthenticateAndPersistOnlyHashes(t *testing.T) {
	...
	if actor.ID != "person-one" || actor.Type != "human" || actor.DisplayName != "Person One" {
...
func TestOpaqueActorIdentifiersFollowTheKernelContract(t *testing.T) {
	...
	if actor.ID != options.Principals[0].ActorID || actor.Type != options.Principals[0].ActorType {
...
func TestSecurityContextChangeInvalidatesExistingCredentials(t *testing.T) {
	...
	if actor, err := changedService.AuthenticateToken(context.Background(), testToken); err != nil || actor.Type != "service" {
...
func TestPasswordRotationCannotRaceAStaleLoginIntoAValidSession(t *testing.T) {
	...
	case <-time.After(identityStoreSynchronizationTimeout):
...
func TestUnchangedProvisioningPreservesPasswordAuthenticationFreshness(t *testing.T) {
	now := time.Date(2026, 8, 4, 9, 0, 0, 0, time.UTC)
	options := provisionedOptions(now)
	_, service := openService(t, options)
	service.clock = func() time.Time { return now }
	authentication, err := service.AuthenticatePassword(context.Background(), "person@example.test", testPassword)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := service.CreateSession(context.Background(), authentication); err != nil {
		t.Fatalf("unchanged provisioning invalidated a current password authentication: %v", err)
	}
}
...
// These test-only helpers exercise the public split contracts as one local
// password browser flow without restoring the removed production interface.
func (service *service) Login(ctx context.Context, username, password string) (identity.Session, error) {
	authentication, err := service.AuthenticatePassword(ctx, username, password)
	if err != nil {
		return identity.Session{}, err
	}
	return service.CreateSession(ctx, authentication)
}

func (service *service) Logout(ctx context.Context, token string) error {
	return service.RevokeSession(ctx, token)
}
